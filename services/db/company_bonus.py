...
class CompanyBonusMixin:
    ...
    async def get_company_bonus_watcher(self, discord_user_id: str) -> dict | None:
        """Return the watcher row for a user, or None if not subscribed."""
        async with self._conn.execute(
            "SELECT discord_user_id, discord_username, game_username, game_user_id, guild_id, added_at "
            "FROM company_bonus_watchers WHERE discord_user_id = ?",
            (discord_user_id,),
        ) as cur:
    ...
    async def get_company_move_advice_watcher(self, discord_user_id: str) -> dict | None:
        async with self._conn.execute(
            "SELECT discord_user_id, discord_username, game_username, game_user_id, guild_id, added_at "
            "FROM company_move_advice_watchers WHERE discord_user_id = ?",
            (discord_user_id,),
        ) as cur:
            row = await cur.fetchone()
            if row is None:
